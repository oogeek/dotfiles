...
marshallPP s pp = pp { ppRename = ppRename pp . unmarshallW, ppSort = (. workspacesOn s) <$> ppSort pp }
...
-- This marshallSort has a bug: see https://github.com/xmonad/xmonad-contrib/issues/420

-- | To still sort by 'VirtualWorkspaces', you can use 'marshallSort' like this:
-- > pp' :: ScreenId -> PP -> PP
-- > pp' s pp = (marshallPP s pp) { ppSort = fmap (marshallSort s) (ppSort pp) }
-- > logHook = let log screen handle = dynamicLogWithPP . pp' screen . pp $ handle
-- >           in log 0 hLeft >> log 1 hRight
--
