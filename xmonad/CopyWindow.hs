...
module XMonad.Actions.CopyWindow (
                                 -- * Usage
                                 -- $usage
                                 copy, copyToAll, copyWindow, runOrCopy, killAllOtherCopies, kill1
                                 , copiesOfOn, taggedWindows
...
-- $logHook
--
-- To distinguish workspaces containing copies of the focused window, use the functions below.
-- First use 'copiesPP' that takes a pretty printer and makes it aware of copies.
-- Then use 'copiesPP' with 'XMonad.Hooks.StatusBar.statusBarProp' :
--
-- > mySB = statusBarProp "xmobar" (copiesPP xmobarPP)
-- > main = xmonad $ withEasySB mySB defToggleStrutsKey def

-- | Takes a pretty printer and makes it aware of copies.
...
killAllOtherCopies = do ss <- gets windowset
    ...
    where
      delFromAllButCurrent w ss = foldr (delWinFromWorkspace w . W.tag) ss (W.hidden ss ++ map W.workspace (W.visible ss))
